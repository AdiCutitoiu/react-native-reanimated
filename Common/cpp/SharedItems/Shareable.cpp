...
void ShareableValue::adapt(jsi::Runtime &rt, const jsi::Value &value, ValueType objectType) {
  if (objectType == MutableValueType) {
    ...
    mutableValue = std::make_shared<MutableValue>(rt, value, module, shared_from_this());
...
std::shared_ptr<ShareableValue> ShareableValue::adapt(jsi::Runtime &rt, const jsi::Value &value, NativeReanimatedModule *module, ValueType valueType) {
  if (value.isObject()) {
    auto object = value.asObject(rt);
    if (object.isHostObject<ShareableHostProxyObject>(rt)) {
      // TODO: maybe check "valueType" if it is inline with host object type
      return object.asHostObject<ShareableHostProxyObject>(rt)->shareableValue;
    }
  }
...
//  if (module->runtime.get() == &rt) {
//    // UI runtime (remote)
//    if (remoteValue.get() == nullptr) {
//      remoteValue = std::unique_ptr<jsi::Value>(new jsi::Value(rt, toJSValue(rt)));
//    }
//    return jsi::Value(rt, *remoteValue);
//  } else {
//    // RN runtime (host)
//    if (hostValue.get() == nullptr) {
//      hostValue = std::unique_ptr<jsi::Value>(new jsi::Value(rt, toJSValue(rt)));
//    }
  ...
  switch (type) {
    ...
    case RemoteObjectType:
      if (module->isUIRuntime(rt)) {
        // remote object accessed from UI thread
        if (remoteObject.get() == nullptr) {
          remoteObject = std::make_shared<RemoteObject>(frozenObject->shallowClone(rt));
        }
        return createHost(rt, remoteObject);
      } else {
        // remote object accessed from JS thread
        return createHost(rt, frozenObject);
      }
      break;
    ...
    case HostFunctionType:
      if (hostRuntime == &rt) {
        ...
        auto clb = [module, hostFunction, hostRuntime](
          ...
          module->scheduler->scheduleOnJS([hostFunction, hostRuntime] {
            hostFunction->call(*hostRuntime);
          });
          return jsi::Value::undefined();
        };
...
void MutableValue::set(jsi::Runtime &rt, const jsi::PropNameID &name, const jsi::Value &newValue) {
  ...
  if (propName == "value") {
    module->valueSetter->getValue(rt)
      .asObject(rt)
      .asFunction(rt)
      .callWithThis(rt, weakSelf.lock()->getValue(rt).asObject(rt), newValue);
...
MutableValue::MutableValue(jsi::Runtime &rt, const jsi::Value &initial, NativeReanimatedModule *module, std::weak_ptr<ShareableValue> weakSelf):
module(module), value(ShareableValue::adapt(rt, initial, module)), weakSelf(weakSelf) {}
...
void ShareableHostProxyObject::set(jsi::Runtime &rt, const jsi::PropNameID &name, const jsi::Value &value) {
  host->set(rt, name, value);
}

jsi::Value ShareableHostProxyObject::get(jsi::Runtime &rt, const jsi::PropNameID &name) {
  return host->get(rt, name);
}
...
std::vector<jsi::PropNameID> RemoteObject::getPropertyNames(jsi::Runtime &rt) {
  ...
  for (int i = 0, size = propertyNames.size(rt); i < size; i++) {
